...
Twitter.prototype.updateStatusWithMedia = function(status, media_name, media_row, encoding, params, callback) {
  if (status === undefined) {
    throw new Error('Missing required parameter: status.');
  }

  if (media_name === undefined) {
    throw new Error('Missing required parameter: media_name.');
  }

  if (media_row === undefined) {
    throw new Error('Missing required parameter: media_row.');
  }

  if (typeof encoding === 'function') {
    callback = encoding;
    encoding = 'base64'; // default encodingf
    params = null;
  }
  ...
  // oauth - multipart post
  var crlf = '\r\n';
  var boundary = 'ece072908565ge59x89ch983452389hosdfd8923y523hedhf923y53';
  var separator = '--' + boundary;
  var footer = crlf + separator + '--' + crlf;
  var fileHttpHeader = 'Content-Disposition: form-data; name="media[]"; filename="' + media_name + '"';
  if (encoding === 'base64') {
    fileHttpHeader = fileHttpHeader + crlf + 'Content-Transfer-Encoding: base64';
    media_row = new Buffer(media_row); // convert base64 string to binary
  }
  ...
  var authorization = this.oauth.authHeader(
    ...
    this.options.access_token_key, this.options.access_token_secret, 'POST');
  var headers = {
...
// Added by Eric Cao
Twitter.prototype.getListsStatuses = function(id, screen_name, params, callback) {
  if (typeof params === 'function') {
    callback = params;
    params = null;
  }
